...
like $entry->id, qr{tag:localhost,\d{4}-\d\d-\d\d:/entrycollection/entry_1.atom};
...
is $feed->title, 'Diary';
ok $feed->updated;
like $feed->id, qr{tag:localhost,\d{4}-\d\d-\d\d:/entrycollection};
...
is $feed->title, 'Photo';
ok $feed->updated;
like $feed->id, qr{tag:localhost,\d{4}-\d\d-\d\d:/mediacollection};
